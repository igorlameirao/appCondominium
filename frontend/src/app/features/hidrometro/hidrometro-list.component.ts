      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Id Unidade</th><th>Número de Série</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.idUnidade }}</td><td>{{ item.numeroSerie }}</td>
              </tr>
            </tbody>
          </table>
...
export class HidrometroListComponent implements OnInit {
  paged: PagedResult<Hidrometro> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  constructor(private service: HidrometroService, private router: Router) {}
  ...
  goNovo(): void { this.router.navigate(['/hidrometro', 'novo']); }
  goEdit(item: Hidrometro): void {
    this.router.navigate(['/hidrometro', item.idHidrometro]);
  }
}
