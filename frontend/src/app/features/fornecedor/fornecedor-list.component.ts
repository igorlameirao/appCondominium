      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>IdPessoa</th><th>Id Condomínio</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.idPessoa }}</td><td>{{ item.idCondominio }}</td>
              </tr>
            </tbody>
          </table>
...
export class FornecedorListComponent implements OnInit {
  paged: PagedResult<Fornecedor> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  goNovo(): void { this.router.navigate(['/fornecedor', 'novo']); }
  goEdit(item: Fornecedor): void {
    this.router.navigate(['/fornecedor', item.idPessoa]);
  }
}
