      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>IdArquivoCobrancaRetorno</th><th>SequencialRegistroRetorno</th><th>IdArquivoCobrancaRemessa</th><th>SequencialRegistroRemessa</th><th>IdUnidade</th><th>DataGeracao</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.idArquivoCobrancaRetorno }}</td><td>{{ item.sequencialRegistroRetorno }}</td><td>{{ item.idArquivoCobrancaRemessa }}</td><td>{{ item.sequencialRegistroRemessa }}</td><td>{{ item.idUnidade }}</td><td>{{ item.dataGeracao }}</td>
              </tr>
            </tbody>
          </table>
...
export class CobrancaListComponent implements OnInit {
  paged: PagedResult<Cobranca> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  constructor(private service: CobrancaService, private router: Router) {}
  ...
  goNovo(): void { this.router.navigate(['/cobranca', 'novo']); }
  goEdit(item: Cobranca): void {
    this.router.navigate(['/cobranca', item.idCobranca]);
  }
}
