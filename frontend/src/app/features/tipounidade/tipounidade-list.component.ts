      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Descrição</th><th>Quantidade de Salas</th><th>Quantidade de Quartos</th><th>Quantidade de Suítes</th><th>Quntidade de Banheiros</th><th>Área</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.descricao }}</td><td>{{ item.qtdSalas }}</td><td>{{ item.qtdQuartos }}</td><td>{{ item.qtdSuites }}</td><td>{{ item.qtdBanheiros }}</td><td>{{ item.area }}</td>
              </tr>
            </tbody>
          </table>
...
export class TipoUnidadeListComponent implements OnInit {
  paged: PagedResult<TipoUnidade> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  constructor(private service: TipoUnidadeService, private router: Router) {}
  ...
  goNovo(): void { this.router.navigate(['/tipounidade', 'novo']); }
  goEdit(item: TipoUnidade): void {
    this.router.navigate(['/tipounidade', item.idTipoUnidade]);
  }
}
