...
@Component({
  selector: 'app-guardaencomenda-list',
  standalone: true,
  imports: [CommonModule, FormsModule, PaginationComponent],
  template: `
    <div class="list-page">
      <div class="list-header">
        <h2>Guarda Encomenda</h2>
      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Id Unidade</th><th>Codigo da Encomenda</th><th>Id Local Armazenamento</th><th>Data/Hora Entrada</th><th>Data/Hora Saída</th><th>Id Colaborador Entrada</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.idUnidade }}</td><td>{{ item.codigoEncomenda }}</td><td>{{ item.idLocal }}</td><td>{{ item.dataHoraEntrada }}</td><td>{{ item.dataHoraSaida }}</td><td>{{ item.idPessoaEntrada }}</td>
              </tr>
            </tbody>
          </table>
...
export class GuardaEncomendaListComponent implements OnInit {
  paged: PagedResult<GuardaEncomenda> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  constructor(private service: GuardaEncomendaService, private router: Router) {}
  ...
  goNovo(): void { this.router.navigate(['/guardaencomenda', 'novo']); }
  goEdit(item: GuardaEncomenda): void {
    this.router.navigate(['/guardaencomenda', item.idGuardaEncomenda]);
  }
}
