      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>IdFornecedor</th><th>Id Concorrência</th><th>idTipoDespesa</th><th>DataDespesa</th><th>Descricao</th><th>DataLancamento</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.idFornecedor }}</td><td>{{ item.idConcorrencia }}</td><td>{{ item.idTipoDespesa }}</td><td>{{ item.dataDespesa }}</td><td>{{ item.descricao }}</td><td>{{ item.dataLancamento }}</td>
              </tr>
            </tbody>
          </table>
...
export class DespesaListComponent implements OnInit {
  paged: PagedResult<Despesa> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  goNovo(): void { this.router.navigate(['/despesa', 'novo']); }
  goEdit(item: Despesa): void {
    this.router.navigate(['/despesa', item.idDespesa]);
  }
}
