      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>CodigoBanco</th><th>DigitoVerificador</th><th>Nome</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.codigoBanco }}</td><td>{{ item.digitoVerificador }}</td><td>{{ item.nome }}</td>
              </tr>
            </tbody>
          </table>
...
export class BancoListComponent implements OnInit {
  paged: PagedResult<Banco> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  goNovo(): void { this.router.navigate(['/banco', 'novo']); }
  goEdit(item: Banco): void {
    this.router.navigate(['/banco', item.codigoBanco]);
  }
}
