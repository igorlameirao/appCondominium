      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Descrição</th><th>Id Local Pai</th><th>Tipo de Local</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.descricao }}</td><td>{{ item.idLocalPai }}</td><td>{{ item.tipoLocal }}</td>
              </tr>
            </tbody>
          </table>
...
export class LocalListComponent implements OnInit {
  paged: PagedResult<Local> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  goNovo(): void { this.router.navigate(['/local', 'novo']); }
  goEdit(item: Local): void {
    this.router.navigate(['/local', item.idLocal]);
  }
}
