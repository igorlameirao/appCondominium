...
@Component({
  selector: 'app-tipomaterial-list',
  standalone: true,
      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Descrição</th><th>Classificacao</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.descricao }}</td><td>{{ item.classificacao }}</td>
              </tr>
            </tbody>
          </table>
...
export class TipoMaterialListComponent implements OnInit {
  paged: PagedResult<TipoMaterial> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  goNovo(): void { this.router.navigate(['/tipomaterial', 'novo']); }
  goEdit(item: TipoMaterial): void {
    this.router.navigate(['/tipomaterial', item.idTipoMaterial]);
  }
}
