      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Código IBGE Município</th><th>Município</th><th>UF</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.codigoIBGEMunicipio }}</td><td>{{ item.nome }}</td><td>{{ item.siglaUF }}</td>
              </tr>
            </tbody>
          </table>
...
export class MunicipioListComponent implements OnInit {
  paged: PagedResult<Municipio> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  constructor(private service: MunicipioService, private router: Router) {}
  ...
  goNovo(): void { this.router.navigate(['/municipio', 'novo']); }
  goEdit(item: Municipio): void {
    this.router.navigate(['/municipio', item.codigoIBGEMunicipio]);
  }
}
