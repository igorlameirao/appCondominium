...
@Component({
  ...
  template: `
    <div class="form-layout">
      <h2>{{ isEdit ? 'Editar' : 'Novo' }} Equipamento</h2>
      <div class="form-scroll">
        <form id="entityForm" (ngSubmit)="submit(f)" #f="ngForm">
          ...
          <div class="form-group">
            <label>ValorAluguel *</label>
            <input type="number" [(ngModel)]="model.valorAluguel" name="valorAluguel" step="0.001" [readonly]="fieldDisabled('valorAluguel')" />
          </div>
          <div class="form-group">
            <label>DuracaoHoras *</label>
            <input type="number" [(ngModel)]="model.duracaoHoras" name="duracaoHoras" [readonly]="fieldDisabled('duracaoHoras')" />
          </div>
          </div>
        </form>
      </div>
...
export class EquipamentoFormComponent implements OnInit {
  ...
  get pkDisplay(): string { return String(this.model.idEquipamento); }
  ...
  ngOnInit(): void {
    ...
    if (idEquipamento) {
      this.isEdit = true;
      this.service.getById(idEquipamento).subscribe({
        next: (r) => this.model = r,
  ...
  submit(f: NgForm): void {
    ...
    if (this.isEdit) {
      this.service.update(this.model).subscribe({
        next: () => this.router.navigate(['/equipamento']),
  ...
  cancel(): void { this.router.navigate(['/equipamento']); }

  confirmDelete(): void {
    this.service.delete(this.model.idEquipamento!).subscribe({ next: () => this.router.navigate(['/equipamento']) });
  }
