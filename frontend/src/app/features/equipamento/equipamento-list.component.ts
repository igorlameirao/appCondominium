      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Descricao</th><th>ValorAluguel</th><th>DuracaoHoras</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.descricao }}</td><td>{{ item.valorAluguel }}</td><td>{{ item.duracaoHoras }}</td>
              </tr>
            </tbody>
          </table>
...
export class EquipamentoListComponent implements OnInit {
  paged: PagedResult<Equipamento> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  goNovo(): void { this.router.navigate(['/equipamento', 'novo']); }
  goEdit(item: Equipamento): void {
    this.router.navigate(['/equipamento', item.idEquipamento]);
  }
}
