      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Descricao</th><th>TipoRateio</th><th>CodigoContabil</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.descricao }}</td><td>{{ item.tipoRateio }}</td><td>{{ item.codigoContabil }}</td>
              </tr>
            </tbody>
          </table>
...
export class TipoDespesaListComponent implements OnInit {
  paged: PagedResult<TipoDespesa> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  goNovo(): void { this.router.navigate(['/tipodespesa', 'novo']); }
  goEdit(item: TipoDespesa): void {
    this.router.navigate(['/tipodespesa', item.idTipoDespesa]);
  }
}
