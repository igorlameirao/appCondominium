...
import { TipoMaterialCondominioService } from './tipomaterialcondominio.service';
import { TipoMaterialCondominio } from './tipomaterialcondominio.model';

@Component({
  selector: 'app-tipomaterialcondominio-list',
  standalone: true,
  imports: [CommonModule, FormsModule, PaginationComponent],
  template: `
    <div class="list-page">
      <div class="list-header">
        <h2>Tipo de Material - Condomínio</h2>
      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Id do Tipo de Material</th><th>Id Condomínio</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.idTipoMaterial }}</td><td>{{ item.idCondominio }}</td>
              </tr>
            </tbody>
          </table>
...
export class TipoMaterialCondominioListComponent implements OnInit {
  paged: PagedResult<TipoMaterialCondominio> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  constructor(private service: TipoMaterialCondominioService, private router: Router) {}
  ...
  goNovo(): void { this.router.navigate(['/tipomaterialcondominio', 'novo']); }
  goEdit(item: TipoMaterialCondominio): void {
    this.router.navigate(['/tipomaterialcondominio', item.idTipoMaterial, item.idCondominio]);
  }
}
