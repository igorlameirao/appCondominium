...
@Component({
  selector: 'app-pessoacondominio-list',
  standalone: true,
  imports: [CommonModule, FormsModule, PaginationComponent],
  template: `
    <div class="list-page">
      <div class="list-header">
        <h2>Pessoa Condominio</h2>
      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>IdPessoa</th><th>DataInicio</th><th>TipoVinculo</th><th>DataFim</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.idPessoa }}</td><td>{{ item.dataInicio }}</td><td>{{ item.tipoVinculo }}</td><td>{{ item.dataFim }}</td>
              </tr>
            </tbody>
          </table>
...
export class PessoaCondominioListComponent implements OnInit {
  paged: PagedResult<PessoaCondominio> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  constructor(private service: PessoaCondominioService, private router: Router) {}
  ...
  goNovo(): void { this.router.navigate(['/pessoacondominio', 'novo']); }
  goEdit(item: PessoaCondominio): void {
    this.router.navigate(['/pessoacondominio', item.idPessoa, item.idCondominio]);
  }
}
