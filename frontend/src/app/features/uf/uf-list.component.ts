      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Sigla UF</th><th>Estado</th><th>Código IBGE</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.siglaUF }}</td><td>{{ item.estado }}</td><td>{{ item.codigoIBGE }}</td>
              </tr>
            </tbody>
          </table>
...
export class UFListComponent implements OnInit {
  paged: PagedResult<UF> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  goNovo(): void { this.router.navigate(['/uf', 'novo']); }
  goEdit(item: UF): void {
    this.router.navigate(['/uf', item.siglaUF]);
  }
}
