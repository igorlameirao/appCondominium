...
@Component({
  selector: 'app-configuracaocondominio-list',
  standalone: true,
  imports: [CommonModule, FormsModule, PaginationComponent],
  template: `
    <div class="list-page">
      <div class="list-header">
        <h2>Configuração do Condomínio</h2>
      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Cor Padrão</th><th>Id Configuração</th><th>Id Condomínio</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.corPadrao }}</td><td>{{ item.idConfiguracao }}</td><td>{{ item.idCondominio }}</td>
              </tr>
            </tbody>
          </table>
...
export class ConfiguracaoCondominioListComponent implements OnInit {
  paged: PagedResult<ConfiguracaoCondominio> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  constructor(private service: ConfiguracaoCondominioService, private router: Router) {}
  ...
  goNovo(): void { this.router.navigate(['/configuracaocondominio', 'novo']); }
  goEdit(item: ConfiguracaoCondominio): void {
    this.router.navigate(['/configuracaocondominio', item.idConfiguracao]);
  }
}
