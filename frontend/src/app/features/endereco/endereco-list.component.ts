      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>CEP</th><th>Logradouro</th><th>Id Bairro</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.cEP }}</td><td>{{ item.logradouro }}</td><td>{{ item.idBairro }}</td>
              </tr>
            </tbody>
          </table>
...
export class EnderecoListComponent implements OnInit {
  paged: PagedResult<Endereco> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  goNovo(): void { this.router.navigate(['/endereco', 'novo']); }
  goEdit(item: Endereco): void {
    this.router.navigate(['/endereco', item.idEndereco]);
  }
}
