...
@Component({
  selector: 'app-aluguelequipamento-list',
  standalone: true,
      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>IdUnidade</th><th>IdEquipamento</th><th>DataInicio</th><th>DataFim</th><th>Valor</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.idUnidade }}</td><td>{{ item.idEquipamento }}</td><td>{{ item.dataInicio }}</td><td>{{ item.dataFim }}</td><td>{{ item.valor }}</td>
              </tr>
            </tbody>
          </table>
...
export class AluguelEquipamentoListComponent implements OnInit {
  paged: PagedResult<AluguelEquipamento> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  constructor(private service: AluguelEquipamentoService, private router: Router) {}
  ...
  goNovo(): void { this.router.navigate(['/aluguelequipamento', 'novo']); }
  goEdit(item: AluguelEquipamento): void {
    this.router.navigate(['/aluguelequipamento', item.idAluguelEquipamento]);
  }
}
