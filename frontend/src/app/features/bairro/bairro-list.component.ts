      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Bairro</th><th>Município</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.nome }}</td><td>{{ item.codigoIBGEMunicipio }}</td>
              </tr>
            </tbody>
          </table>
...
export class BairroListComponent implements OnInit {
  paged: PagedResult<Bairro> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  constructor(private service: BairroService, private router: Router) {}
  ...
  goNovo(): void { this.router.navigate(['/bairro', 'novo']); }
  goEdit(item: Bairro): void {
    this.router.navigate(['/bairro', item.idBairro]);
  }
}
