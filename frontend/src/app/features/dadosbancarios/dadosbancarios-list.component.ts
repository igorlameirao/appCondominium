...
@Component({
  selector: 'app-dadosbancarios-list',
  standalone: true,
      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Conta</th><th>CodigoBanco</th><th>Agencia</th><th>DigitoAgencia</th><th>DigitoConta</th><th>Ativo</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.conta }}</td><td>{{ item.codigoBanco }}</td><td>{{ item.agencia }}</td><td>{{ item.digitoAgencia }}</td><td>{{ item.digitoConta }}</td><td>{{ item.ativo }}</td>
              </tr>
            </tbody>
          </table>
...
export class DadosBancariosListComponent implements OnInit {
  paged: PagedResult<DadosBancarios> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  constructor(private service: DadosBancariosService, private router: Router) {}
  ...
  goNovo(): void { this.router.navigate(['/dadosbancarios', 'novo']); }
  goEdit(item: DadosBancarios): void {
    this.router.navigate(['/dadosbancarios', item.conta, item.codigoBanco, item.agencia]);
  }
}
