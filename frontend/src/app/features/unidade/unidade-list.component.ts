      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>Id Unidade</th><th>Id Bloco</th><th>Número</th><th>Id Tipo Unidade</th><th>Id Hidrômetro</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.idUnidade }}</td><td>{{ item.idBloco }}</td><td>{{ item.numero }}</td><td>{{ item.idTipoUnidade }}</td><td>{{ item.idHidrometro }}</td>
              </tr>
            </tbody>
          </table>
...
export class UnidadeListComponent implements OnInit {
  paged: PagedResult<Unidade> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  goNovo(): void { this.router.navigate(['/unidade', 'novo']); }
  goEdit(item: Unidade): void {
    this.router.navigate(['/unidade', item.idUnidade]);
  }
}
