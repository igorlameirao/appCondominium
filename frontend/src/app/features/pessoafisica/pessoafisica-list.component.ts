...
@Component({
  selector: 'app-pessoafisica-list',
  standalone: true,
      ...
      <div class="table-scroll-wrapper">
        <div class="table-container">
          <table *ngIf="paged?.items?.length; else empty">
            <thead><tr><th>IdPessoa</th><th>Data de Nascimento</th><th>e-mail</th></tr></thead>
            <tbody>
              <tr *ngFor="let item of paged?.items" (click)="goEdit(item)" class="row-click">
                <td>{{ item.idPessoa }}</td><td>{{ item.dataNascimento }}</td><td>{{ item.email }}</td>
              </tr>
            </tbody>
          </table>
...
export class PessoafisicaListComponent implements OnInit {
  paged: PagedResult<Pessoafisica> | null = null;
  loading = false;
  page = 1;
  pageSize = 20;
  filterText = '';
  ...
  goNovo(): void { this.router.navigate(['/pessoafisica', 'novo']); }
  goEdit(item: Pessoafisica): void {
    this.router.navigate(['/pessoafisica', item.idPessoa]);
  }
}
