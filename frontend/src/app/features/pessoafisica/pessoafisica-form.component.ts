      ...
      <div class="form-scroll">
        <form id="entityForm" (ngSubmit)="submit(f)" #f="ngForm">
          ...
          <div class="form-group">
            ...
            <input type="date" [(ngModel)]="model.dataNascimento" name="dataNascimento" [readonly]="fieldDisabled('dataNascimento')" />
          </div>
          <div class="form-group">
            <label>e-mail</label>
            <input type="text" [(ngModel)]="model.email" name="email" maxlength="255" [readonly]="fieldDisabled('email')" />
          </div>
          <div class="form-group">
            <label>Senha</label>
            <input type="password" [(ngModel)]="model.senha" name="senha" autocomplete="new-password" />
          </div>
          </div>
        </form>
      </div>
...
export class PessoafisicaFormComponent implements OnInit {
  model: Pessoafisica = {} as Pessoafisica;
  isEdit = false;
  saving = false;
  formError = '';
  ...
  submit(f: NgForm): void {
    ...
    if (this.isEdit) {
      this.service.update(this.model).subscribe({
        next: () => this.router.navigate(['/pessoafisica']),
  ...
  cancel(): void { this.router.navigate(['/pessoafisica']); }

  confirmDelete(): void {
    this.service.delete(this.model.idPessoa!).subscribe({ next: () => this.router.navigate(['/pessoafisica']) });
  }
